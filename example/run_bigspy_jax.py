#!/usr/bin/env python3
"""
bigspy JAX Demo — JAX-accelerated two-stage spectral fitting pipeline

Usage:
    python example/run_bigspy_jax.py

Features:
    - JAX JIT-compiled likelihood (12-80x faster on CPU)
    - Same API as NumPy version, just add use_jax=True
    - All plots and results identical to run_bigspy.py
"""
import os, sys, time
import numpy as np
...
# ╚══════════════════════════════════════════════════════════════════╝
print("=" * 60)
print("  bigspy JAX — Bayesian Inference of Galaxy Spectra")
print("=" * 60)
...
mc = MCMCFitter(
    ...
    sfh_model="delayed",
    wave_range=(3600, 7400), use_jax=True,
)
...
mc_dpl = MCMCFitter(
    ...
    sfh_model=DoublePowerLawSFH, wave_range=(3600, 7400), use_jax=True,
)
...
print(f"\n{'=' * 60}")
print("  bigspy JAX Demo Complete")
print(f"{'=' * 60}")
